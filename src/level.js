...
// side-scrolling level from the Lumina game. You steer the acorn with on-screen buttons (or
// the keyboard); tapping the Glowcap switches its light on and off, and the "bloom"
// platforms are only there, and only solid, while it shines on them.
//
// The level is much wider than the stage, so the stage is a window that scrolls with the
// player (as the game's own camera does), clipped at its edges so it reads as a diorama box.
...
import {createPlatformer, PLAYER} from './platformer'
...
const GLOWCAP_HEIGHT = 1.2      // game units
...
const TRAIL_SECONDS = 2.5       // how long a light puddle left by the player takes to fade
...
// about 'won' and 'respawn' so the caller can show a message.
export const createLevel = ({glowLight, onEvent = () => {}, data = LIGHT_THE_WAY}) => {
  ...
  const blooms = data.blooms.map((spec) => {
    ...
    const bloom = {spec, x: spec.x, y: spec.y, w: spec.w, h: spec.h, solid: false, alpha: GHOST_ALPHA, mesh, strip}
    solids.push(bloom)
    return bloom
  })

  // ---- puddles of light the player leaves behind on whatever it walks on ----
  ...
  const glowcapCentre = {x: data.glowcap.x, y: data.glowcap.y + 0.6}
  ...
  lightDisc.position.set(glowcapCentre.x, glowcapCentre.y, -1.5)
  lightDisc.raycast = () => {}
  ...
  // ---- the physics, the two characters, and the scrolling camera ----
  const platformer = createPlatformer({
    spawn: data.spawn,
    fallY: data.fallY,
    exit: {...data.exit, r: data.exitTouchRadius},
  })
  const st = platformer.state
  const minCam = data.bounds.minX + VIEW_HALF
  const maxCam = data.bounds.maxX - VIEW_HALF
  // The opening shot frames both the acorn and the Glowcap, so the Glowcap is on screen (and
  // tappable) from the start. The camera holds that shot until the player first moves, then
  // only follows once the player gets within `edgeMargin` of the stage's edge.
  const startCam = clamp((data.spawn.x + data.glowcap.x) / 2, minCam, maxCam)
  ...
  glowcapGroup.position.set(data.glowcap.x, data.glowcap.y, 0)
  glowcapGroup.visible = false
  content.add(glowcapGroup)

  // A generous invisible box to tap on: the Glowcap is tiny on the stage, and a fingertip is not.
  const glowcapHit = new THREE.Mesh(new THREE.BoxGeometry(3, 3, 3), new THREE.MeshBasicMaterial())
  glowcapHit.position.y = 0.9
  ...
  const loadGlowcap = () => {
    new GLTFLoader().load(glowcapUrl, (gltf) => {
      ...
      const size = box.getSize(new THREE.Vector3())
      const center = box.getCenter(new THREE.Vector3())
      const scale = GLOWCAP_HEIGHT / size.y
      model.scale.setScalar(scale)
      model.position.set(-center.x * scale, -box.min.y * scale, -center.z * scale)
      finishModel(model)
      ...
      glow = createGlow(glowcapGroup, model, glowLight, {haloSize: 3.4, haloY: 0.7, lightY: 0.8, lightDistance: 2.2})
  ...
  return {
    ...
    // If the ray from a tap hits the Glowcap, switches its light and returns true.
    tapGlowcap(raycaster) {
      if (!active || !glow || raycaster.intersectObject(glowcapHit, false).length === 0) {
        return false
      }
      glow.target = glow.target ? 0 : 1
      playSound(glow.target ? 'glowOn' : 'glowOff')
      return true
    },
    ...
    update(dt, t, input) {
      ...
      // The light decides which bloom platforms exist. A bloom is solid while the Glowcap is
      // switched on and within that platform's radius: the same rule as the game's
      // night_bloom_platform.gd (distance to the Glowcap <= light_radius).
      const lightOn = glow.target === 1
      blooms.forEach((bloom) => {
        const distance = Math.hypot(bloom.x - glowcapCentre.x, bloom.y - glowcapCentre.y)
        bloom.solid = lightOn && distance <= bloom.spec.radius
      })
      ...
      keyJump = false
      platformer.step(dt, {dir, jumpPressed}, solids)

      for (const event of platformer.drainEvents()) {
        if (event === 'jump') {
          playSound('boing')
          startJump(false)
      ...
      // Animation: idle, run, or the jump pose while off the ground.
      if (!st.onGround) {
      ...
      } else if (dir !== 0 && Math.abs(st.vx) > 0.4) {
        actions.run.timeScale = Math.max(0.6, Math.abs(st.vx) / PLAYER.walkSpeed)
      ...
      const moving = dir !== 0 || !st.onGround
      yaw += (st.facing * (moving ? 1.3 : 0.75) - yaw) * (1 - Math.exp(-dt * 12))
      playerGroup.position.set(st.x, st.y, 0)
      playerGroup.rotation.y = yaw

      // Light puddles under its feet.
      if (st.onGround && st.groundedOn && Math.abs(st.x - lastPuddleX) > TRAIL_SPACING) {
        dropPuddle(st.x, st.groundedOn.y + st.groundedOn.h / 2)
      }
      ...
      // Bloom platforms fade toward solid or ghost, and glow while solid.
      blooms.forEach((bloom) => {
        const target = bloom.solid ? 1 : GHOST_ALPHA
        ...
        bloom.mesh.castShadow = bloom.alpha > 0.6
      ...
      // Scroll the level under the stage: hold still until the player nears an edge, then
      // follow just enough to keep them `edgeMargin` inside it. The window is centred a bit
      // ahead of the way the acorn faces, so more of what it is running toward shows.
      if (dir !== 0 || jumpPressed) {
        started = true
      }
      if (started) {
        const slack = VIEW_HALF - edgeMargin
        const centre = st.x + st.facing * 1.5
